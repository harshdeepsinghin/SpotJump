...
function injectButton() {
    ...
    if (document.querySelector('.spoti-search-btn')) return;
    ...
    if (likeDislikeContainer && likeDislikeContainer.parentElement) {
        ...
        if (!likeDislikeContainer.isConnected) {
            console.log('SpotiSearch: Like/Dislike container found but not connected to DOM');
            return;
        }

        console.log('SpotiSearch: Found Like/Dislike container, injecting button...');

        const btn = document.createElement('button');
        btn.className = 'spoti-search-btn';
        btn.title = 'Search on Spotify';
        // Use a pill shape style to match the actions bar
        btn.innerHTML = `
            <span class="spoti-icon">
            ...
            <span class="spoti-text">Spotify</span>
        `;
        ...
        try {
            ...
            console.log('SpotiSearch: Button injected successfully');
        } catch (e) {
            console.error('SpotiSearch: Error injecting button:', e);
        }
    } else {
        console.log('SpotiSearch: Like/Dislike container not found yet');
    }
}
...
function startInjectionLoop() {
    ...
    injectionInterval = setInterval(() => {
        attempts++;
        if (document.querySelector('.spoti-search-btn')) {
        ...
        if (attempts > 20) { // Stop after 10 seconds (20 * 500ms)
            clearInterval(injectionInterval);
            console.log('SpotiSearch: Gave up injecting button after 10 seconds');
            return;
        }
...
// Initial check
if (window.location.href.includes('/watch')) {
    console.log('SpotiSearch: Initial check started');
    startInjectionLoop();
} else {
    console.log('SpotiSearch: Not a watch page');
}

// Handle navigation events specifically for YouTube SPA
window.addEventListener('yt-navigate-finish', () => {
    console.log('SpotiSearch: yt-navigate-finish detected');
    if (window.location.href.includes('/watch')) {
        startInjectionLoop();
    }
});
